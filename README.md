...
npm run package   # → build/chrome/ and build/firefox/, each loadable
```
...
The two targets need different manifests — Firefox has no `background.service_worker` — so both are generated by `scripts/manifest.js` from `manifest.base.json`. There is no checked-in `manifest.json` to edit.
